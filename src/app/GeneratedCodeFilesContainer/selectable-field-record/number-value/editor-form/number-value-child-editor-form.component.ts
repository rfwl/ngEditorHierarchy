...
import { SelectableOptionRecord } from '../../../selectable-option-record/selectable-option-record.model'
  ...
  @Component({
    selector: "number-value-child-editor-form", 
templateUrl: './number-value-child-editor-form.component.html', 

styleUrls: ['./number-value-child-editor-form.component.scss'] 

}) 

    export class NumberValueChildEditorFormComponent implements OnInit {
...
try {
  ...
  this.store.pipe(select(selectAllSelectableOptionRecords)).subscribe (entLst => {
    ...
    outputJsonString(){
      ...
      show_CodeOutput_Modal(this.modalService, 'NumberValue', str, 'modal-xl')
    }
