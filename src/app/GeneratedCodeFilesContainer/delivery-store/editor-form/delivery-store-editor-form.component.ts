  ...
  import { EnumSelectorOptionSource } from '../delivery-store.model'
...
import { AddressChildEditorFormComponent } from "../address/editor-form/address-child-editor-form.component";
  ...
  @Component({
    selector: "delivery-store-editor-form", 
templateUrl: './delivery-store-editor-form.component.html', 

styleUrls: ['./delivery-store-editor-form.component.scss'] 

}) 


  export class DeliveryStoreEditorFormComponent implements OnInit {
    firestore: Firestore
    ...
    outputJsonString(){
      ...
      show_CodeOutput_Modal(this.modalService, 'DeliveryStore', str, 'modal-xl')
    }
    ...
    get editorFormTitle(): string{
      switch(this.editorOperationType){
        case EnumEditorOperationType.Create:
          return 'Create DeliveryStore'
        case EnumEditorOperationType.Delete:
          return 'Delete DeliveryStore'
        case EnumEditorOperationType.Update:
          return 'Update DeliveryStore'
        default:
          return 'Edit DeliveryStore'
      }
    }
...
get Managers() { return this.editorForm.get('Managers')  }
    ...
    submitEditorForm(editorForm:any){
      ...
      switch(this.editorOperationType){
        case EnumEditorOperationType.Create:
          ...
          // this.store.dispatch(DeliveryStoreActionTypes.createDeliveryStore({ deliveryStore: {...this.editedObject } }))
          break
        case EnumEditorOperationType.Delete:
          if(this.parentDataArray && this.parentDataIndex >= 0){
            this.parentDataArray?.splice(this.parentDataIndex,1)
            // moved the next line to Editor AgGrid
            // this.store.dispatch(DeliveryStoreActionTypes.deleteDeliveryStore({ deliveryStoreId: this.editedObject.id }))
          }
          break
        case EnumEditorOperationType.Update:
          break
