  ...
  import { EnumSelectorOptionSource } from '../school-room.model'
  ...
  @Component({
    selector: "school-room-editor-form", 
templateUrl: './school-room-editor-form.component.html', 

styleUrls: ['./school-room-editor-form.component.scss'] 

}) 


  export class SchoolRoomEditorFormComponent implements OnInit {
    firestore: Firestore
    ...
    outputJsonString(){
      ...
      show_CodeOutput_Modal(this.modalService, 'SchoolRoom', str, 'modal-xl')
    }
    ...
    get editorFormTitle(): string{
      switch(this.editorOperationType){
        case EnumEditorOperationType.Create:
          return 'Create SchoolRoom'
        case EnumEditorOperationType.Delete:
          return 'Delete SchoolRoom'
        case EnumEditorOperationType.Update:
          return 'Update SchoolRoom'
        default:
          return 'Edit SchoolRoom'
      }
    }
...
get Capacity() { return this.editorForm.get('Capacity')  }
    ...
    submitEditorForm(editorForm:any){
      ...
      switch(this.editorOperationType){
        case EnumEditorOperationType.Create:
          ...
          // this.store.dispatch(SchoolRoomActionTypes.createSchoolRoom({ schoolRoom: {...this.editedObject } }))
          break
        case EnumEditorOperationType.Delete:
          if(this.parentDataArray && this.parentDataIndex >= 0){
            this.parentDataArray?.splice(this.parentDataIndex,1)
            // moved the next line to Editor AgGrid
            // this.store.dispatch(SchoolRoomActionTypes.deleteSchoolRoom({ schoolRoomId: this.editedObject.id }))
          }
          break
        case EnumEditorOperationType.Update:
          break
